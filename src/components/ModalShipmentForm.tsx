...
const ModalTravelForm = ({ modalForm, setModalForm }: Modal) => {
  ...
  const { errorMessage, shipment } = useAppSelector((state) => state.shipment);
  const {  travels } = useAppSelector((state) => state.travel);
  const { startNewShipment, startEditShipment } = useShipment();
  ...
  useEffect(() => {
    if (shipment?._id) {
      setValues({
        _id: shipment._id,
        travel: shipment.travel?._id as unknown as Travel,
        from:shipment.from,
        to:shipment.to,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (shipment?._id) {
      await startEditShipment(values);
    } else {
      await startNewShipment(values);
    }
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                  ...
                  >
                    {shipment?._id ? "Edit Shipment" : "New Shipment"}
                  </Dialog.Title>
                  <form onSubmit={handleSubmit} className="my-10" action="">
                    <div className="mb-5">
                      <label htmlFor="travel" className="font-bold text-m">
                        Travel
                      </label>
                      <select
                        name="travel"
                        id="travel"
                        value={values.travel?.toString()}
                      ...
                      >
                        ...
                        {travels.map((travel) => (
                          <option key={travel._id} value={travel._id}>
                           {formatDate(travel.date as Date)} - {travel.driver?.name} {travel.driver?.lastname} {travel.assistant ? '-':''} {travel.assistant?.name} {travel.assistant?.lastname} - {travel.vehicle?.patent} {travel.semirremolque ? '-':''} {travel.semirremolque?.patent}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <input
                      type="submit"
                      value={shipment?._id ? "Save Changes" : "Save Shipment"}
